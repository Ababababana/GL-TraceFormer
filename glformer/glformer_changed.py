...
from efg.operators import nms_gpu, boxes_iou3d_gpu
from efg.resnet.resnet_mmdet import ResNet
...
class TrajectoryFormer(nn.Module):
    def __init__(self, config):
        ...
        if self.config.projection != None:
            ...
            self.num_cameras = self.config.projection.num_cameras
            model_cfg = dict(
                  depth=34,
                  num_stages=3,
                  strides=(1,2,2,),
                  dilations=(1,1,1,),
                  out_indices=(2,),    #下采样倍率 ：8， 16 ，32 ；   通道数： 128， 256，512
                  frozen_stages=3,
                  norm_cfg=dict(type='BN', requires_grad=True),
                  norm_eval=True,
                  style='pytorch',
                  init_cfg='https://s3.amazonaws.com/pytorch/models/resnet50-19c8e357.pth'
            )

            self.model_2d=ResNet(**model_cfg)
            self.fusion_enc_fg = self.config.projection.enc_fg
            self.fusion_enc_gl = self.config.projection.enc_gl
            self.downsample_2d =nn.MaxPool2d(kernel_size=8,stride=8,padding=0)
        ...
        self.cls_embed = MLP(
            self.hidden_dim * 3 + 3, self.hidden_dim, self.hidden_dim, 3
        )
    ...
    def forward_train(self, batched_inputs):
        ...
        samples = collate([bi[0][0] for bi in batched_inputs], self.device)

        if self.projection:
            samples_img = [bi[0][1]["images"] for bi in batched_inputs]
            calibrations=[bi[0][1]["calibrations"] for bi in batched_inputs]
            poses = [bi[0][1]["poses"]  for bi in batched_inputs]
        ...
        if self.is_train:
            ...
            if self.num_track > 0 and det_boxes3d.shape[1] > 0:
                ...
                if self.projection:
                    batched_2d_features= self.get_img_features(global_candidates,samples_img,calibrations,poses)

                point_feat_list = self.get_trajcetory_point_feature(
                    global_trajectory_hypothses, samples,batched_2d_features
                )
                ...
                bs,num_boxes,patches,channels = batched_2d_features.shape
                #feature_2d_to_downsample =batched_2d_features.transpose(-2,-1).view(-1,channels,8,8)
                downsample_2d_features = self.downsample_2d(batched_2d_features.transpose(-2,-1).view(-1,channels,8,8))
                downsample_2d_features =downsample_2d_features.view(bs,-1,self.num_hypo_train,channels)                

                hypotheses_feat = self.get_trajectory_hypotheses_feat(
                    point_feat_list, boxes_feat, pred_labels,downsample_2d_features
                )
                ...
                point_reg = self.point_reg(torch.cat(point_feat_list, 0)).reshape(
                    1, self.batch_size * self.num_track, self.num_hypo_train, 7
                )
    ...
    def forward_inference(self, batched_inputs):
        ...
        if self.frame_id == 0:
            ...
            if self.projection:
                batched_2d_features= self.get_img_features(global_candidates,samples_img,calibrations,poses)# [bs, num_boxes, h*w , channels]
            batched_2d_features = batched_2d_features.reshape(-1,batched_2d_features.shape[-2],batched_2d_features.shape[-1])
            ...
            point_feat = torch.cat([point_feat,batched_2d_features],dim=1)
            ...
            num_boxes,patches,channels = batched_2d_features.shape
            #feature_2d_to_downsample =batched_2d_features.transpose(-2,-1).view(-1,channels,8,8)
            downsample_2d_features = self.downsample_2d(batched_2d_features.transpose(-2,-1).view(-1,channels,8,8))
            downsample_2d_features =downsample_2d_features.view(self.batch_size,-1,self.num_hypo_inference,channels)


            hypotheses_feat = self.get_trajectory_hypotheses_feat_inference(
                point_feat_list, boxes_feat, self.instances[-1].pred_classes,downsample_2d_features
            )
            feat_list = self.encoder_globallocal(hypotheses_feat)
            hypotheses_scores = (
    ...
    def clip(self,a,mini,maxi):
        a_min=np.where(a<mini,mini,a)
        return np.where(a_min>maxi,maxi,a)


    def project_vehicle_to_image(self,vehicle_pose, calibration, points):      
        ...
        # Populate camera image metadata. Velocity and latency stats are filled with zeroes.
    ...
    def projection_2d(self,boxes_3D,calibrations,poses):
        batch_size,_,num_objs,obj_hypos,feat_dim=boxes_3D.shape
        boxes_3d_list=boxes_3D.view(batch_size,num_objs*obj_hypos,feat_dim)[:,:,:7]
        boxes_per_batch=[]
        for batch_id,boxes_3d in enumerate(boxes_3d_list):
            frame_calib=calibrations[batch_id]
            frame_pose = poses[batch_id]
            boxes_per_frame=[] 
           
            for camera_id in range(self.num_cameras):
                camera_calib = frame_calib[camera_id]
                boxes_per_camera = []
                for box_id,box in enumerate(boxes_3d):
                    box_coords=box.unsqueeze(0).float()
                    corners = box_utils.get_upright_3d_box_corners(box_coords.cpu().numpy())[0].numpy()
                    projected_corners = self.project_vehicle_to_image(frame_pose, camera_calib,corners)
                    u, v, ok = projected_corners.transpose()
                    ok = ok.astype(bool)
                    if not all(ok):
                        boxes_per_camera.append([-1,]*4)
                        continue
                    u=u[ok]
                    v=v[ok]

                    # Clip box to image bounds.
                    u = self.clip(u, 0, camera_calib.width)
                    v = self.clip(v, 0, camera_calib.height)
                    if u.max() - u.min() == 0 or v.max() - v.min() == 0:
                        boxes_per_camera.append([-1,]*4)
                        continue
                    boxes_per_camera.append([u.min(),v.min(),u.max(),v.max()])
                assert len(boxes_per_camera) == num_objs * obj_hypos
                boxes_per_frame.append(torch.tensor(boxes_per_camera))
            boxes_per_frame = torch.stack(boxes_per_frame,dim=1)   # [num_boxes, 5,4]
            
            boxes_per_batch.append(boxes_per_frame)
        boxes_per_batch = torch.stack(boxes_per_batch,dim=0)    
        valid_mask = (boxes_per_batch == -1).all(dim=(3))  # shape: [batch_size,boxes,5]
        return boxes_per_batch,valid_mask     # Tensor [batch_size,boxes_in_img, 5, 4],



    def batched_datas(self,boxes_per_batch,samples_img,camera_id):
        boxes_list = [ frame_box[:,camera_id].float().cuda() for frame_box in boxes_per_batch ] #List [ Tensor[N,4] ]*batch_size
        image_list = torch.stack([ batch_img[camera_id] for batch_img in samples_img ],dim=0)
        return boxes_list,image_list.cuda()


    #def rearrange_roi_feats(self,batched_feats,len_list)：
        #for i inrange(len(len_list)):
            


    def get_sinusoid_encoding(self,num_patches, hidden_dim):
        position_enc = torch.zeros(num_patches, hidden_dim)
        ...
        return position_enc.cuda()


    def img_feature_extraction(self,boxes_per_batch,samples_img,valid_mask):
        with torch.no_grad():
            batch_roi_feats=[]
            for camera_id in range(self.num_cameras):
                batched_boxes, batched_img = self.batched_datas(boxes_per_batch,samples_img,camera_id)
                feature_map = self.model_2d(batched_img)[0]
                roi_feats =roi_align(input=feature_map.float(),boxes=batched_boxes,output_size=8,spatial_scale=1.0/16.0)   # roi_features of one camera position in the batch,   shape= [all_boxes_in_batch,c,h,w]
                batch_roi_feats.append(roi_feats)
            batched_feats =torch.stack(batch_roi_feats,dim=1)   # [all_boxes,5,c,h,w]
           
            valid_feats = batched_feats.view(self.batch_size,-1,batched_feats.shape[1],batched_feats.shape[2],batched_feats.shape[3],batched_feats.shape[4])      #[batch_size, boxes_in_img, 5, c, h, w]
            valid_feats[~valid_mask] = 0
            valid_num = torch.sum(valid_mask.float(),dim=1)
            valid_id  = torch.argmax(valid_num,dim=-1)
            output_feats=[]
            for j,v_id in enumerate(valid_id):
                output_feats.append(valid_feats[j,:,v_id])
        return torch.stack(output_feats,dim=0) #[batch_size, boxes,c,h,w]
            
              

    def get_img_features(self,boxes_3d,samples_img,calibrations,poses):
        if not self.is_train:
            boxes_3d=boxes_3d[None,None]
        boxes_per_batch,valid_mask = self.projection_2d(boxes_3d,calibrations,poses)
        roi_features   = self.img_feature_extraction(boxes_per_batch,samples_img,valid_mask)
        batch_size,num_boxes,roi_channels,roi_h,roi_w = roi_features.shape
        roi_features   = roi_features.view(batch_size,num_boxes,roi_channels,roi_h*roi_w).transpose(2,3) # [bs, num_boxes, h*w , channels]
        pos_embding    = self.get_sinusoid_encoding(roi_features.shape[2],roi_features.shape[3])
        return roi_features + pos_embding
    ...
    def get_trajcetory_point_feature(self, global_trajectory_hypothses, samples,batched_2d_features):
        ...
        image_feat = batched_2d_features.reshape(-1,batched_2d_features.shape[-2],batched_2d_features.shape[-1])
        point_feat =torch.cat([point_feat,image_feat],dim= 1)
    ...
    def get_trajectory_hypotheses_feat(self, point_feat_list, boxes_feat, pred_labels,downsample_2d_features):
        point_feat = point_feat_list[-1].reshape(
            self.batch_size, self.num_track, self.num_hypo_train, -1
        )
        src = torch.cat(
            [point_feat, boxes_feat, downsample_2d_features,torch.zeros_like(point_feat)[..., :3]], -1
        )
    ...
    def get_trajectory_hypotheses_feat_inference(
        self, point_feat_list, boxes_feat, pred_labels,downsample_2d_features
    ):
        point_feat = point_feat_list[-1].reshape(
            self.batch_size, self.num_track, -1, self.token.shape[-1]
        )

        downsample_2d_features = downsample_2d_features.reshape(
            self.batch_size, self.num_track, -1, downsample_2d_features.shape[-1]
        )
    ...
    def get_loss(
        ...
        #f=open("/home/cjzhang/EFG/playground/tracking.3d/waymo/trajectoryformer/trajectoryformer.mppnet/loss_log.txt","a")
        #f.write('\n'*6)
        ...
        #f.write(f" loss_corner:      {loss_corner}" + "\n")
        ...
        #f.write(f" loss_box_cls:      {loss_box_cls}" + "\n")
        ...
        #f.write(f" loss_joint_cls:      {loss_joint_cls}" + "\n")
        #f.close()

        loss_bj_cls = loss_box_cls + loss_joint_cls
        if torch.isnan(loss_bj_cls):
            loss_bj_cls = 0

        if torch.isnan(loss_reg):
            loss_reg = 0        
        if torch.isnan(loss_corner):
            loss_corner = 0

        loss_cls = loss_point_cls + loss_bj_cls
