...
class glFormer(nn.Module):
    ...
    def forward_train(self, batched_inputs):
        ...
        # if frame_id >0, the input will have 4 history frame sweeps with their gt_boxes, the cur_frame(input) will have both its pred_boxes and its gt_boxes
        ...
        if self.is_train:
            ...
            ###  把list拼成tensor， 并且基于线性匀速运动假设缝合历史轨迹（10历史帧，1当前帧）
    ...
    def forward_inference(self, batched_inputs):
        ...
        if self.frame_id == 0:
            ...
            # get history boxes from history_bank,(t-1 ~t-11), all boxes coordinates are in global_coord, turn it all into current frame's veh coord (watching the 10 history frames in current frame view  
    ...
    def get_history_traj(self, cur_ids):
        ...
        # using cur frame t as anchor, each hypo_future_frame is generate from t-i frame 
        ...
        for k, id in enumerate(cur_ids):
            ...
            #since is using history boxes which all have been transfered into global coords, now turn them back to veh_coord for prediction
    ...
    def get_proposal_aware_point_feature(self, src, trajectory_rois, num_rois):
        ...
        # src is the current frame's detections and its augs
        for i in range(trajectory_rois.shape[1]):
            ...
            #   corner_points .shape = [bs, num_rois (== num_tracks * num_candidates), 8 corners, coordinate of each corner points]
            ...
            # Nine key points of bboxs : [8 corners + 1 center]
            corner_add_center_points = torch.cat(
                [corner_points, trajectory_roi_center], dim=-1
            )
            # calculate the distance of each cropped points in the bbox and the 9 key points (related pos) 
    ...
    def get_trajectory_boxes_feature(self, traj_rois):
        ...
        # get agent box of future traj hypo through pointnet-like network
	...
	# generate the past history traj (t-1 ~t-10)
    ...
    def generate_trajectory_hypothses(
        ...
        # L2 distance between the real detection of cur frame and the pred result of cur frame from t-1 frame
        ...
        #select the topk(k==1) matched real detection bboxs of the t-1 frame pred bboxs
        ...
        # Find out the real detection bboxes of cur frame which can be matched with the pred from t-1 frame
        ...
        # concat "pred_bboxs_feom_t-1 frame"  and  "augmentation hypos of cur frame"  and "real detection of cur frame"
        # get all the candidates of the cur frame,   then concate cur frame with the 9 future frame (t+1 ~ t+10)
    ...
    def get_cls_targets(self, pred_boxes3d, global_candidates, targets):
        ...
        for i in range(batch_size):
            ...
            if pred_boxes3d[i].shape[0] > 0 and num_gt > 0:
                ...
                # Find out the most matched gt_box for each pred_box
                max_iou, track_id = track_iou.max(-1)
                fg_track_mask = max_iou > 0.5
 
                # Regression all the pred_boxes which has iou with gt_boxes >0.5
                reg_mask = rois_iou.max(-1)[0] > 0.5
        
                # Based on the iou between pred_boxes and gt_box, select out the group_iou (pred_box itself,its two augs, and its correspond real det) and gt_boxes
                group_iou = rois_iou[
    ...
    def get_iou_labels(self, cls_iou):
        ...
        # soft-label of middle iou targets
	...
	# Based on linear invariant velocity movement, assum the history traj and fill it with IOU matched history frame detections
    ...
    def get_loss(
        ...
        f=open("/home/cjzhang/EFG/playground/tracking.3d/waymo/trajectoryformer/trajectoryformer.centerpoint/loss_log.txt",'a')
        f.write('\n'*5)
        ...
        f.write(f" loss_cls (total):   {loss_cls}"+'\n')
        f.write(f" loss_reg (total):   {loss_reg}"+'\n'*3)
        f.close()
    ...
    def get_pred_motion(self, traj, pred_vel=None):
        ...
        # Under current frame's view, transfer all the history traj_boxes and future traj_pred_boxes into the relative coord of t-1 frame's boxes
        ...
        # transfer the linear movement prediction trajectory based on t-1 frame into the relative position of t-1 frame under current frame's view (as prior boxes), adjusted by the voxel prediction follow
    ...
    def genereate_trajcetory_hypotheses_inference(
        ...
        #cat the multi hypo of current frame (both real current frame detection and prediction from t-1 frame) with the t-9 ~ t-1 frame traj;
        # equals using current frame as the anchor, then concate the 9 history traj with it as the future pridiction traj
