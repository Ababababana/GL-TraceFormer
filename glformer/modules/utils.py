...
def crop_current_frame_points(num_lidar_points, trajectory_rois, points):
    ...
    # only crop current frame's lidar points
    for bs_idx in range(batch_size):
        cur_batch_boxes = trajectory_rois[bs_idx, 0, :, :7].view(-1, 7)

        # calculate the radius of each bbox
        cur_radiis = (
            torch.sqrt(
        ...
        if cur_batch_boxes.shape[0] > 16:
            ...
            for i in range(length_iter + 1):
                ...
                # calculate the L2 distance of all the valid points of this frame and  the center of detction bboxs
                dis = torch.norm(
                    (
        ...
        # select out the points that L2 distance < radius
        point_mask = dis <= cur_radiis.unsqueeze(-1)


        # if selcted points > num_lidar_points (128): random choose;   if <: pad zero;  if None: all pad zeros
